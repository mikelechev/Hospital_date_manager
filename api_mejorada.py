# -*- coding: utf-8 -*-
"""
Created on Sun Jul 12 11:18:35 2026
...
import pandas as pd
import uvicorn
import xgboost as xgb  # Asegúrate de tener este import arriba del todo

# AHORA (Formato seguro .json):
...
if os.path.exists(MODELO_PATH):
    ...
    modelo_ia = None
    print("⚠️ ATENCIÓN: No se encontró el archivo. Se usará simulación matemática para la demo.")
    ...
    def book_slot(self, hora, nombre_paciente, prob_ausencia):
        ...
        # CASO 2: Hueco ocupado -> Filtro matemático de Overbooking
            ...
            if prob_ambos_vienen < 0.25 and prob_al_menos_uno > 0.8:
                self._insertar(hora, nombre_paciente, prob_ausencia)
                return True, f"⚠️ OVERBOOKING INTELIGENTE APROBADO en {hora} (Riesgo choque: {prob_ambos_vienen*100:.0f}%)."
            else:
                return False, f"⛔ BLOQUEADO en {hora}: Riesgo de colapso excesivo en sala de espera."
                
        # CASO 3: Límite absoluto de 2 pacientes por hueco
        else:
            return False, f"⛔ BLOQUEADO en {hora}: El hueco está completamente saturado."

# --- 3. MODELOS DE DATOS (Lo que envía la web) ---
# --- 3. MODELOS DE DATOS (Con valores por defecto para no saturar la web) ---
class PeticionCita(BaseModel):
    nombre: str
    hora: str
    
    # Variables que sí pediremos o cambiaremos fácilmente
    age: int = 35
    days_between: int = 14
    ratio_faltas: float = 0.1
    sms_received: int = 1
    weekend: int = 0
    
    # Variables clínicas secundarias (Las rellenamos por defecto a 0 = "No")
    # Así la API se las manda al modelo sin que tú tengas que teclearlas en la demo
    scholarship: int = 0
    hipertension: int = 0
    diabetes: int = 0
    alcoholism: int = 0
    handcap: int = 0
    gender_m: int = 0  # 0 = Mujer, 1 = Hombre
...
# --- 5. LA CARARA: INTERFAZ WEB PROFESIONAL (FRONTEND) ---
    ...
    <body class="bg-slate-900 text-gray-100 font-sans min-h-screen p-6">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                <!-- Panel de Entrada de Datos (5 Columnas) -->
                <div class="md:col-span-5 bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                    <h2 class="text-xl font-bold text-white mb-4 flex items-center gap-2">
                        <span>👤 Evaluar Nuevo Paciente</span>
                    </h2>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-slate-300 text-sm font-semibold mb-1">Nombre del Paciente:</label>
                            <input type="text" id="nombre" class="w-full bg-slate-900 border border-slate-600 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500" placeholder="Ej: Carlos Mendoza">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-slate-300 text-sm font-semibold mb-1">Días de Espera:</label>
                                <input type="number" id="dias" class="w-full bg-slate-900 border border-slate-600 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500" value="14">
                            </div>
                            <div>
                                <label class="block text-slate-300 text-sm font-semibold mb-1">Faltas Previas:</label>
                                <input type="number" id="faltas" class="w-full bg-slate-900 border border-slate-600 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500" value="2">
                            </div>
                        </div>

                        <div>
                            <label class="block text-slate-300 text-sm font-semibold mb-1">¿Es Cita en Fin de Semana?:</label>
                            <select id="finde" class="w-full bg-slate-900 border border-slate-600 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500">
                        ...
                        <div>
                            <label class="block text-blue-400 text-sm font-bold mb-1">Seleccionar Hora:</label>
                            <select id="hora" class="w-full bg-slate-900 border border-blue-500 rounded px-3 py-2 text-white font-mono focus:outline-none">
                        ...
                        <button onclick="procesarPaciente()" class="w-full mt-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3 px-4 rounded-lg shadow-lg transform transition active:scale-95 flex justify-center items-center gap-2">
                            <span>⚡ Evaluar IA & Agendar</span>
                        </button>
                    </div>

                    <!-- Caja de Resultado IA -->
                    <div id="panel-resultado" class="mt-6 p-4 rounded-lg bg-slate-900/50 border border-slate-700 hidden">
                ...
                <div class="md:col-span-7 bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                    ...
                    <div id="lista-agenda" class="space-y-3">
                        <!-- Aquí se inyectan las tarjetas de tiempo -->
                    </div>
                </div>
            </div>
        </div>
...
def evaluar_y_reservar(peticion: PeticionCita):
    ...
    # IMPORTANTE: Reconstruimos las 13 columnas EXACTAS con las que entrenó tu IA
    # (Excluimos 'No-show' porque es lo que la IA va a predecir, no un dato de entrada)
    df_paciente = pd.DataFrame([{
        ...
        'Days-between': peticion.days_between,      # Ojo al guión medio que tienes aquí
    ...
    # 2. IA predice el riesgo usando TU modelo de 13 variables
    if modelo_ia is not None:
        try:
            prob_ausencia = float(modelo_ia.predict_proba(df_paciente)[0][1])
        except Exception as e:
            print(f"❌ ERROR CON EL MODELO: {e}")
            # Si algo falla por nombres de columnas, avisamos en la consola y usamos plan B
            prob_ausencia = min(0.95, (peticion.ratio_faltas * 0.7) + (peticion.days_between * 0.01))
    else:
        prob_ausencia = min(0.95, (peticion.ratio_faltas * 0.7) + (peticion.days_between * 0.01))

    # 3. Mandamos al paciente al cortafuegos del calendario
    exito, mensaje = agenda.book_slot(peticion.hora, peticion.nombre, prob_ausencia)
    
    return {
        "exito": exito,
        "probabilidad": prob_ausencia,
        "mensaje": mensaje
    }
