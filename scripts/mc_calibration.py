# -*- coding: utf-8 -*-
"""
Simulación mensual de agenda hospitalaria con efectos de cola (Efecto Dominó),
Cortafuegos, Calibración Isotónica y Análisis de Trade-off (Umbrales).
"""
...
print("🏥 INICIANDO SIMULACIÓN Y OPTIMIZACIÓN DE AGENDA HOSPITALARIA...")
print("-" * 75)
...
FEATURES = [
    ...
    "Handcap", "SMS_received", "Days_between", "Weekend", 
    "Ratio_Faltas", "Gender_M", "Scheduled_Time_of_Day_Evening", 
    "Scheduled_Time_of_Day_Morning"
]

def prepare_dataset_and_model():
    """Carga el dataset, el modelo y aplica la calibración isotónica en RAM."""
    ...
    if df["No-show"].dtype == "object":
        df["Falta_Real"] = df["No-show"].map({"Yes": True, "No": False, "1": True, "0": False})
    else:
        df["Falta_Real"] = df["No-show"] == 1

    # Cargar y Calibrar Modelo
    if not MODEL_PATH.exists():
        print(f"❌ ERROR: No se encuentra '{MODEL_PATH}'.")
        exit()

    modelo_base = xgb.XGBClassifier()
    modelo_base.load_model(str(MODEL_PATH))
    print("✅ CEREBRO XGBOOST CARGADO")

    print("🔥 CALIBRANDO PROBABILIDADES EN VIVO (Regresión Isotónica)...")
    modelo_ia = CalibratedClassifierCV(estimator=modelo_base, method="isotonic", cv="prefit")
    modelo_ia.fit(df[FEATURES], df["Falta_Real"])
    print("✅ MODELO CALIBRADO CON ÉXITO\n")

    # Calcular probabilidad de todo el dataset de forma segura
...
def create_schedule(day_patients_pool, smart_overbooking=False, umbral_riesgo=0.40):
    ...
    # 1. Asignación base (1 paciente por hueco, saltando cortafuegos)
    ...
    # 2. IA Smart-Slotting (Overbooking si Prob_IA > umbral)
...
def simulate_day_queue(agenda):
    ...
    for slot in range(TOTAL_SLOTS):
        ...
        if pacientes_citados and len(pacientes_que_llegan) == 0 and cola == 0:
            slots_status[slot] = 0 # Gris: No show absoluto
        
        cola += len(pacientes_que_llegan)
        
        if cola > 0:
            if len(pacientes_que_llegan) > 0 and cola == len(pacientes_que_llegan):
                slots_status[slot] = 1 # Verde: A tiempo
            else:
                slots_status[slot] = 2 # Naranja: Retrasado por cola anterior
            
            cola -= 1
            atendidos += 1
        else:
            if not pacientes_citados:
                slots_status[slot] = 3 # Blanco: Hueco libre / Cortafuegos
...
def plot_tradeoff_curve(umbrales, atendidos_extra, solapamientos, path):
    """Genera la gráfica de compromiso (Trade-off) entre pacientes rescatados y fricción."""
    fig, ax1 = plt.subplots(figsize=(10, 6))

    x = [str(u) for u in umbrales]

    color = '#2ecc71'
    ax1.set_xlabel('Umbral de Decisión ($\theta$)', fontsize=12, fontweight='bold')
    ...
    plt.title('Curva de Compromiso (Trade-off): Beneficio vs. Riesgo de Colapso', fontsize=14, fontweight='bold', pad=15)
...
def main():
    ...
    # 1. Simulación Tradicional de referencia (¡Arreglado el error de matrix_trad!)
    ...
    # 2. Barrido de Sensibilidad (5 Umbrales)
    ...
    mejor_umbral = None
    max_score = -float('inf')  # Usamos Score matemático en lugar de solo máximo de pacientes
    mejor_matrix = None
    ...
    for umbral in umbrales_a_probar:
        ...
        # Función de Optimización (Beneficio vs Penalización por solapamiento)
        score = (extra * 1.0) - (stats_ia['solapamientos'] * 0.5)

        print(f"   🎯 Umbral > {umbral:.2f} | Atendidos: {stats_ia['atendidos']} (+{extra} extra) | Solapamientos: {stats_ia['solapamientos']} | Score Óptimo: {score:.1f}")

        # Ahora elije el mejor umbral basándose en la rentabilidad matemática
        if score > max_score:
            max_score = score
            mejor_umbral = umbral
